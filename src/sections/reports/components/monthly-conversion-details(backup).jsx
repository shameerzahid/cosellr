...
// GO TO THE SAME FILE and start commenting pieces in vite project
// you will see component desapearing
// leave the minimum useful and then ask copilot to remove the commented code
// in this part, this is the part displaying the email !
...
export function MonthlyConversionDetails({ mail, renderLabel, error, loading }) {
  ...
  const renderHead = () => (
    <>
      ...
      <Box sx={{ gap: 1, flexGrow: 1, display: 'flex' }}>
        {(mail?.labelIds?.length > 0 ? mail.labelIds : ['placeholder']).map((labelId, index) => {
          const label =
            labelId === 'placeholder'
              ? { id: 'placeholder', name: 'Static Label', color: '#007BFF' }
              : renderLabel?.(labelId);
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        {/* Buttons at the corner : Archive, Mark Unread, Trash... */}
